var PourOver = (function() {
  ...
  _.extend(PourOver.Filter.prototype, PourOver.Events, {
    ...
    removeSingleQuery: function(q, silent) {
      ...
      var s = [],
        stack = this.current_query.stack,
        new_stack,
        is_compound = function(c) {
    ...
    getByPossibilityGroups: function() {
      ...
      return _.reduce(this.possibilities, function(m, p, k) {
        m[k] = collection.get(p.matching_cids);
        return m;
      }, {});
    }
  });
  ...
  _.extend(PourOver.View.prototype, PourOver.Events, {
    ...
    removeSort: function() {
      if (this.current_sort.off) {
        this.current_sort.off("resort");
      }
  ...
  _.extend(PourOver.MatchSet.prototype, PourOver.Events, {
    ...
    slice: function(s, e) {
      return this.collection.get(this.cids.slice(s, e));
    },
  ...
  _.extend(PourOver.UI.Element.prototype, PourOver.Events, {
    ...
    getSimpleSelectState: function(match_set, s, output) {
      if (typeof(match_set) === "undefined" || !match_set || !match_set.stack) {
        return false;
      }
      if (typeof(s) === "undefined") {
        s = match_set.stack;
    ...
    getSimpleRangeState: function(match_set) {
      ...
      if (stack.length !== 1 || stack[0][1].length !== 2) {
        throw "The filter specified does not appear to have a simple range; stack.";
      }
      return stack[0][1];
    }
  ...
  PourOver.BufferedCollection = PourOver.Collection.extend({
    ...
    stripFutures: function(item) {
      return _.reduce(item, function(m, v, k) {
    ...
    bufferGuids: function(guids) {
      var that = this;
      guids = _.select(guids, function(g) {
  ...
  PourOver.manualFilter = PourOver.Filter.extend({
    ...
    getFn: function(query) {
      if (_.isArray(query)) {
        ...
        return new PourOver.MatchSet(query, this.getCollection(), [
          [this, query]
        ]);
  ...
  PourOver.exactFilter = PourOver.Filter.extend({
    ...
    getFn: function(query) {
      ...
      if (_.isArray(query)) {
        ...
        return new PourOver.MatchSet(possibility.matching_cids, this.getCollection(), [
          [this, query]
        ]);
      }
    }
  ...
  PourOver.makeRangeFilter = function(name, ranges, opts) {
    ...
    var values = _.map(ranges, function(r) {
        return {
          low: r[0],
          high: r[1],
          value: r.join("-")
  ...
  PourOver.dvrangeFilter = PourOver.Filter.extend({
    ...
    getFn: function(query) {
      if (!query[0] || !query[1]) {
        return new PourOver.MatchSet([], this.getCollection(), [
          [this, query]
        ]);
      }
      ...
      that = this;
      possibilities = _.map(this.values.slice(li, hi + 1), function(p) {
  ...
  PourOver.UI.SimpleSelectElement = PourOver.UI.Element.extend({
    initialize: function(opts) {
      if (!opts.filter) {
        throw "A simple select element must have a filter specified";
      }
  ...
  PourOver.UI.SimpleDVRangeElement = PourOver.UI.Element.extend({
    initialize: function(opts) {
      if (!opts.filter) {
        throw "A simple dv range element must have a filter specified";
      }
      this.filter = opts.filter;
    },
    getMatchSet: function() {
